...
This includes contextual `await`/`yield`, iteration-header `of`, TypeScript postfix/type-instantiation,
and otherwise-division slash tokens separated from the previous token by a line
terminator. The latter may be automatic-semicolon-insertion boundaries; the
recognizer rejects the ambiguity instead of guessing division. Reserved-word
prefixes such as `extends`, `export default`, and `debugger` preserve regexp
boundaries without confusing properties with those names for keywords. Spread is a single token,
