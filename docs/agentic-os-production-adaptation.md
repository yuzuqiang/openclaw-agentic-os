...
- 2026-08-11: Corrected the installed-runtime preflight evidence model after Phase A found a combined-catalog false negative:
  - Corrections: `scripts/openclaw-tool-capability-preflight.py` now separates model-callable `tools.catalog` evidence from source-bound Gateway RPC evidence. Gateway allowLease registration is corroborated only through the non-mutating `subagents.allowLease.status` call. The 2026-08-09 installed-runtime negative baseline is retained but marked superseded/retracted as a false negative for allowLease RPC availability. New split-catalog evidence shows OpenClaw 2026.7.1 has source-bound `subagents.allowLease.acquire/status/release` registration and status returns `ok=true`, `writeMode=memory`; the preflight still fails closed on the future metadata/idempotency/full-owner fields, missing `sessions_spawn` metadata fields, and the future canonical `sessions_status` alias. `agentic_os.DB_AUTHORITY_ENABLED` remains `False`.
...
- The 2026-08-11 split-catalog evidence in
  ...
  exposes `session_status` while DB authority requires the future canonical
  `sessions_status` alias.
