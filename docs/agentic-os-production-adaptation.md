...
- `sessions_spawn` or its tool-layer wrapper must accept exactly `client_request_id`, `idempotency_key`, `gateway_lease_id`, and `metadata={run_id, phase, agent_id, transition_id, task_digest}` at the top-level request boundary.
