...
the database rejects cross-table source dedupe reuse and freezes the completed
session proof once referenced by a settlement. The current amount/SLO contract
blocks incomplete or extra linked event sets and revalidates completed-session
proof even if a malicious fixture bypasses runtime triggers.
...
WHERE ((bs.actual_time_seconds>0 OR bs.actual_input_tokens>0
   ...
   OR EXISTS (
       SELECT 1 FROM budget_events be
       WHERE be.settlement_id IS NULL
         AND be.run_id=bs.run_id
         ...
         AND be.event_sequence>(
           SELECT COALESCE(MAX(linked.event_sequence),0)
           FROM budget_events linked
           WHERE linked.settlement_id=bs.settlement_id
         ))
   OR NOT EXISTS (
       SELECT 1
