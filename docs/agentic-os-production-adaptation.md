...
| Unknown usage blocks auto-local | `SELECT run_id FROM run_budgets WHERE usage_confidence='unknown' AND run_id IN (SELECT run_id FROM runs WHERE state IN ('gate_passed','release_pending','finalized'));` |
