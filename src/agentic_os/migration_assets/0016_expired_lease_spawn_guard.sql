...
DROP TRIGGER IF EXISTS leases_preserve_live_gateway_expiry_update;
...
AFTER UPDATE OF state, run_id, phase, transition_id, agent_id, requester_agent_id, client_lease_id, release_idempotency_key, gateway_lease_id, expires_at_epoch_ms ON leases
WHEN NEW.state IN ('expired','human_review_required')
  AND (
    (
...
CREATE TRIGGER leases_preserve_live_gateway_expiry_update
BEFORE UPDATE OF expires_at, expires_at_epoch_ms ON leases
WHEN OLD.state IN ('acquired','release_pending')
  AND OLD.gateway_lease_id IS NOT NULL
  AND OLD.gateway_lease_id <> ''
  AND (
...
BEGIN
  SELECT RAISE(ABORT,'live lease expiry is immutable after acquisition');
END;
