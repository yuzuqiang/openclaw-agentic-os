...
class ReviewCompletionTests(unittest.TestCase):
    ...
    def test_export_default_regex_cannot_hide_dynamic_import(self) -> None:
        for quote in ("'", '"'):
            for trivia in (" ", "\n", "/* boundary */", "/*\n*/"):
                for regex in (f"/{quote}/", f"/[{quote}]/"):
                    source = f"export default{trivia}{regex};await import('./hidden.mjs');//{quote}"
                    with self.subTest(source=source):
                        self.assertIn(("./hidden.mjs", True, "import"), CONTRACT.import_specifiers(source))

    def test_statement_regex_boundaries_cannot_hide_require(self) -> None:
        prefixes = (
            "debugger\n",
            "label:while(true){break label\n",
            "label:for(let i=0;i<1;i++){continue label\n",
        )
        for prefix in prefixes:
            suffix = "}" if "label:" in prefix else ""
            for terminator in ("\n", "\r", "\r\n", "\u2028", "\u2029", "/*\n*/"):
                source = prefix.replace("\n", terminator) + '/"/;' + suffix + "require('./hidden.cjs');//\""
                with self.subTest(source=source):
                    self.assert_bound_or_rejected(source)

    def test_contextual_of_and_newline_division_are_explicitly_ambiguous(self) -> None:
        for source in (
            'for(const c of /"/.source){}require(\'./hidden.cjs\');//"',
            "const n=4;n\n/require('./hidden.cjs')/2;",
            "const n=4;n/*\n*//require('./hidden.cjs')/2;",
            'import "node:path"\n/"/;await import(\'./hidden.mjs\');//"',
            'export * from "node:path"\n/"/;await import(\'./hidden.mjs\');//"',
        ):
            with self.subTest(source=source), self.assertRaisesRegex(CONTRACT.RuntimeSourceContractError, "ambiguous"):
                CONTRACT.import_specifiers(source)

    def test_reserved_property_names_and_same_line_division_remain_executable(self) -> None:
        for name in sorted(CONTRACT.JS_REGEX_PREFIX_KEYWORDS | {"await", "yield", "of"}):
            for member in (f"obj.{name}", f"obj?.{name}"):
                source = f"const obj={{{name}:4}};{member} / require('./hidden.cjs');"
                with self.subTest(source=source):
                    self.assertIn(("./hidden.cjs", True, "require"), CONTRACT.import_specifiers(source))
        for source in (
            "const n=4;n / require('./hidden.cjs');",
            "const of=4;of / require('./hidden.cjs');",
            "function f(of){return of / require('./hidden.cjs');}",
            "let of=4;for(of / require('./hidden.cjs');false;){}",
            "let of=4;for(let n=0;of / require('./hidden.cjs');n++){break;}",
            "const n=4;n /\n require('./hidden.cjs');",
            "true / require('./hidden.cjs');",
            "null / require('./hidden.cjs');",
        ):
            with self.subTest(source=source):
                self.assertIn(("./hidden.cjs", True, "require"), CONTRACT.import_specifiers(source))

    @unittest.skipUnless(NODE, "requires Node execution witness")
    def test_node_executes_loads_after_asi_and_default_regex(self) -> None:
        self.write("hidden.cjs", "console.log('BOUND_SLASH_WITNESS');")
        self.write("hidden.mjs", "console.log('BOUND_SLASH_WITNESS');")
        sources = (
            ('main.mjs', 'export default /"/;await import(\'./hidden.mjs\');//"'),
            ('main.cjs', 'debugger\n/"/;require(\'./hidden.cjs\');//"'),
            ('main.cjs', 'label:while(true){break label\n/"/;}require(\'./hidden.cjs\');//"'),
            ('main.cjs', 'label:for(let i=0;i<1;i++){continue label\n/"/;}require(\'./hidden.cjs\');//"'),
            ('main.cjs', 'for(const x of /"/.source){}require(\'./hidden.cjs\');//"'),
            ('main.mjs', 'import "node:path"\n/"/;await import(\'./hidden.mjs\');//"'),
        )
        for filename, source in sources:
            with self.subTest(source=source):
                script = self.write(filename, source)
                proc = subprocess.run([NODE, str(script)], cwd=self.root, capture_output=True, text=True, timeout=10)
                self.assertEqual(proc.returncode, 0, proc.stderr)
                self.assertIn("BOUND_SLASH_WITNESS", proc.stdout)
                try:
                    specifiers = CONTRACT.import_specifiers(source)
                except CONTRACT.RuntimeSourceContractError:
                    continue
                self.assertIn(("./hidden.mjs", True, "import") if filename.endswith(".mjs") else
                              ("./hidden.cjs", True, "require"), specifiers)

    def test_semicolonless_type_does_not_hide_child_process_transfer(self) -> None:
        source = (
            "import{spawnSync}from'node:child_process';type Foo=string\n"
