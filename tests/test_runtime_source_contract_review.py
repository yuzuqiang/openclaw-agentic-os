...
class RuntimeSourceContractReviewTests(unittest.TestCase):
    ...
    def test_combined_default_namespace_imports_track_both_bindings(self) -> None:
        for module, transfer in (
            ("child_process", "const go=ns['spawnSync']; go('./hidden.sh');"),
            ("child_process", "ns.execFileSync('./hidden.sh');"),
            ("worker_threads", "const W=ns.Worker; new W(new URL('./hidden.mjs', import.meta.url));"),
            ("module", "const N=ns.Module; new N().load('./hidden.cjs');"),
        ):
            for prefix in ("", "node:"):
                source = f"import def, * as ns from '{prefix}{module}'; " + transfer
                with self.subTest(source=source), self.assertRaises(CONTRACT.RuntimeSourceContractError):
                    CONTRACT.import_specifiers(source)
        for source in (
            "import wt, * as ns from 'node:worker_threads'; new ns.Worker(new URL('./hidden.cjs', import.meta.url));",
            "import cp, * as ns from 'node:child_process'; ns.spawnSync(process.execPath, ['./hidden.cjs']);",
            "import M, * as ns from 'node:module'; const r=ns.createRequire(import.meta.url); r('./hidden.cjs');",
        ):
            with self.subTest(source=source):
                try:
                    specifiers = CONTRACT.import_specifiers(source)
                except CONTRACT.RuntimeSourceContractError:
                    continue  # Unsupported signatures must not silently omit code.
                self.assertIn('./hidden.cjs', [item[0] for item in specifiers])

    def test_factory_result_escape_is_not_mistaken_for_a_bound_loader(self) -> None:
        for source in (
            "import * as M from 'node:module'; const r=M.createRequire(import.meta.url); r('./hidden.cjs');",
            "const r=require('node:module').createRequire(__filename); r('./hidden.cjs');",
            "import {createRequire as make} from 'node:module'; const box=[make(import.meta.url)]; box[0]('./hidden.cjs');",
            "import {createRequire as make} from 'node:module'; const r=(()=>make(import.meta.url))(); r('./hidden.cjs');",
            "import {createRequire as make} from 'node:module'; const r=make(import.meta.url).bind(null, './hidden.cjs'); r('./decoy.cjs');",
            "import {createRequire as make} from 'node:module'; make(import.meta.url)('./hidden.cjs');",
            "import {createRequire as make} from 'node:module'; const r=make(import.meta.url)\ninstanceof { [Symbol.hasInstance](loader) { return loader('./hidden.cjs'); } };",
            "require('node:module').createRequire(__filename)('node:child_process').execSync('./hidden.sh');",
        ):
            with self.subTest(source=source), self.assertRaises(CONTRACT.RuntimeSourceContractError):
                CONTRACT.import_specifiers(source)
        for source in (
