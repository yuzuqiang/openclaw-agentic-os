...
class RuntimeSourceContractRegressionTests(unittest.TestCase):
    ...
    def test_namespace_and_loader_spread_transfers_are_rejected(self) -> None:
        declarations = (
            ("import * as wt from 'node:worker_threads';", "wt", "new holder.Worker('./hidden.cjs');"),
            ("import * as M from 'node:module';", "M", "new holder.Module().load('./hidden.cjs');"),
            ("import * as cp from 'node:child_process';", "cp", "holder.spawnSync('./hidden.sh');"),
            ("const r=require;", "r", "holder.main.require('./hidden.cjs');"),
            ("const {createRequire}=require('node:module');const r=createRequire(__filename);", "r", "consume(holder);"),
        )
        for declaration, capability, invocation in declarations:
            for gap in ("", " ", "/* comment */", "\n"):
                with self.subTest(declaration=declaration, gap=gap):
                    self.assert_rejected(declaration + "const holder={..." + gap + capability + "};" + invocation)
        # Dot-property names do not refer to an imported binding.
        self.assert_bound("import * as wt from 'node:worker_threads';const data={};data.wt=1;require('./hidden.cjs');")
