...
class RuntimeLexicalTypeBoundaryTests(unittest.TestCase):
    ...
    def test_parsed_type_alias_asi_ends_before_next_runtime_statement(self) -> None:
        for gap in ("\n", "\r", "\r\n", "\u2028", "\u2029", "/*\n*/"):
            declaration = (
            ...
            with self.subTest(gap=gap):
                self.assertIn(("node:worker_threads", True, "import"),
                              SOURCE.import_specifiers(declaration + "void isMainThread;"))
                for tail in (
                    "(W=Worker);new W('./hidden.mjs');",
                    "const W=Worker;new W('./hidden.mjs');",
                    "require('./hidden.cjs');",
                ):
                    source = declaration + tail
                    try:
                        result = SOURCE.import_specifiers(source)
                    except SOURCE.RuntimeSourceContractError:
                        pass
                    else:
                        self.assertEqual(tail, "require('./hidden.cjs');")
